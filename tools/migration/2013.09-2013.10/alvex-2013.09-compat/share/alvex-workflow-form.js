...
(function()
{
    ...
    * @return {Alvex.WorkflowForm} The new WorkflowForm instance
    * @constructor
    */
   Alvex.WorkflowForm = function WorkflowForm_constructor(htmlId)
   {

      Alvex.WorkflowForm.superclass.constructor.call(this, "Alvex.WorkflowForm", htmlId, ["button", "container", "datasource", "datatable"]);
      this.isReady = false;
      this.workflow = null;
      this.currentTasks = [];
      this.historyTasks = [];
   ...
   YAHOO.extend(Alvex.WorkflowForm, Alfresco.component.Base,
   {
      /**
      ...
      _loadWorkflowForm: function WF__loadWorkflowForm()
      {
         if (this.isReady && this.workflow)
         {
            ...
            Dom.get(this.id + "-workflowType").innerHTML = this.msg("label.title") + ': ' + $html(this.workflow.title);
            ...
            Dom.get(this.id + "-workflowMessage").innerHTML = '<h2>' + $html(message) + '</h2>';
            ...
            Dom.get(this.id + "-startedBy").innerHTML = this.msg("label.startedBy") + ':<br/>' + Alfresco.util.userProfileLink(
                  initiator.userName || this.msg("label.usernameDeleted"), initiator.firstName + " " + initiator.lastName, null, !initiator.firstName);

            var initiatorAvatar = initiator.avatarUrl;
            var initiatorLink = Alfresco.util.userProfileLink(
                  initiator.userName || this.msg("label.usernameDeleted"), initiator.firstName + " " + initiator.lastName, null, !initiator.firstName);
            Dom.get(this.id + "-initiatorAvatar").setAttribute("src", initiatorAvatar ? Alfresco.constants.PROXY_URI + initiatorAvatar  + "?c=force" : Alfresco.constants.URL_RESCONTEXT + "components/images/no-user-photo-64.png");
            Dom.get(this.id + "-initiatorCommentLink").innerHTML = this.msg("label.recentTaskOwnersCommentLink", initiatorLink);
            Dom.get(this.id + "-initiatorComment").innerHTML = this.msg("label.noComment");

            var workflowCompletedDate = Alfresco.util.fromISO8601(this.workflow.endDate);
            Dom.get(this.id + "-completedSummary").innerHTML = this.msg("label.completed") + ':<br/>' + $html(workflowCompletedDate ? Alfresco.util.formatDate(workflowCompletedDate, "longDate") + ' ' + Alfresco.util.formatDate(workflowCompletedDate, "shortTime"): this.msg("label.notCompleted"));

            var today = new Date();
            var dueDate = Alfresco.util.fromISO8601(this.workflow.dueDate);
            if (dueDate)
            {
               Dom.get(this.id + "-dueSummary").innerHTML = this.msg("label.due") + ':<br/>' 
                       + (!workflowCompletedDate && today > dueDate ? '<span class="workflow-delayed"></span>' : '')
                       + Alfresco.util.formatDate(dueDate, "longDate");
               // Dom.get(this.id + "-due").innerHTML = Alfresco.util.formatDate(dueDate, "defaultDateOnly");
            }
            else
            {
               Dom.get(this.id + "-dueSummary").innerHTML = this.msg("label.due") + ':<br/>' + this.msg("label.none");
            ...
            if (startDate)
            {
               Dom.get(this.id + "-startedAt").innerHTML = this.msg("label.started") + ':<br/>' + Alfresco.util.formatDate(startDate, "longDate") + ' ' + Alfresco.util.formatDate(startDate, "shortTime");
            }
      ...
      onWorkflowFormLoaded: function WorkflowForm_onWorkflowFormLoaded(response)
      {
         ...
         var formFieldsEl = Selector.query(".form-fields", this.id, true),
            ...
            initialTaskEl = Dom.get(this.id + "-initialTask"),
            initialTaskSpoilerEl = Dom.get(this.id + "-workflow-initial-task");

         for( var i = formFieldsEl.children.length; i > 0; i-- )
         {
            initialTaskEl.appendChild(formFieldsEl.children[0]);
         }

         formFieldsEl.insertBefore(initialTaskSpoilerEl, Dom.getFirstChild(formFieldsEl));
         formFieldsEl.insertBefore(generalSummaryEl, initialTaskSpoilerEl);
      ...
      renderCellComment: function WorkflowForm_renderCellComment(elCell, oRecord, oColumn, oData)
      {
         ...
         if( oRecord.getData("id") == this.workflow.startTaskInstanceId )
         {
            Dom.get(this.id + "-initiatorComment").innerHTML = $html( oRecord.getData("properties").bpm_comment || this.msg("label.noComment") );
         }
      },
