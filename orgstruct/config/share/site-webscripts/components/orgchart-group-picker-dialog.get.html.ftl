...
<div id="${fieldHtmlId}-dialog" class="yui-panel">
	...
	<style type="text/css" media="screen">
		...
		#${fieldHtmlId}-search { width: 160px; }
		...
		<div id="${fieldHtmlId}-container" class="form-container">
			<div id="${fieldHtmlId}-view-selector">
				...
				<span style="padding-left: 80px;">
					<strong>${msg("itd.orgchart.search_by_name")} </strong>
					<input id="${fieldHtmlId}-search" name="people-search" onkeydown="if (event.keyCode == 13) YAHOO.Bubbling.fire('searchClicked')" />
					<button id="${fieldHtmlId}-search-button">${msg("button.search")}</button>
				</span>
			</div>
			<div id="doc4">
				<div class="yui-gb">
