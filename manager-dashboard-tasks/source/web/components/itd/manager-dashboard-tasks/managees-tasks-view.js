...
(function()
{
	...
	*/
	...
	ITD.ManageesTasksViewer = function(htmlId)
	{
		ITD.ManageesTasksViewer.superclass.constructor.call(this, "ManageesTasksViewer", htmlId);
		return this;
	};


	YAHOO.extend(ITD.ManageesTasksViewer, Alfresco.component.Base,
	{
		options:
		{
			managees: [],
		...
		{
			var managees_url = Alfresco.constants.PROXY_URI + "api/itd/orgstruct/" 
			...
			for(var gr in manageesStruct.data)
				for(var u in manageesStruct.data[gr].managees)
					if(manageesStruct.data[gr].managees[u].shortName != Alfresco.constants.USERNAME)
						this.options.managees.push(manageesStruct.data[gr].managees[u]);
			...
			this.widgets.pagingDataTable[m] = new Alfresco.util.DataTable(
			{
				...
				dataSource:
				{
					url: Alfresco.constants.PROXY_URI + "api/task-instances?" 
							+ "authority=" + this.options.managees[m].shortName
		...
		*/
		...
		manageesSortAndUnique: function TL_manageesSortAndUnique(managees)
		{
			...
			for (var i = 1; i < managees.length; i++)
				if (managees[i-1].shortName !== managees[i].shortName)
					ret.push(managees[i]);
			return ret;
		}
	});
