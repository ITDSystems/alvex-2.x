...
(function()
{
    ...
    * @return {Alvex.DataLists} The new DataLists instance
    * @constructor
    */
   Alvex.DataListsSelector = function(htmlId)
   {
      Alvex.DataListsSelector.superclass.constructor.call(this, "Alvex.DataListsSelector", htmlId, ["button", "container"]);
    ...
    */
   YAHOO.extend(Alvex.DataListsSelector, Alfresco.component.Base,
   {
      /**
      ...
      options:
      {
         /**
          ...
          /**
           * If set, show only lists of this type
           *
           * @property listTypesToShow
           * @type String
           */
          listTypesToShow: "",
		  
           /**
           * The type of newly created datalists
           *
           * @property listContainerType
           * @type String
           */
           listContainerType: "dl:dataList",

          /**
           * If we should show actions - new list, list edit and delete
           *
      ...
      populateDataLists: function DataLists_populateDataLists(p_callback)
      {
         ...
         var fnSuccess = function DataLists_pDL_fnSuccess(response, p_obj)
         {
            ...
            {
               list = lists[i];
               if( ( (this.options.listTypesToShow === "") || (list.itemType.match(this.options.listTypesToShow)) ) 
				&& (!list.itemType.match("^alvexdt:object$")) )
                  this.dataLists[list.name] = list;
            }
            this.dataListsLength = this.dataLists.length;
            ...
            else
            {
               ...
               for (var index in lists)
               {
                  if (lists.hasOwnProperty(index))
                  {
                     ...
                     if (permissions["edit"] && (this.options.showActions !== "none") )
                     {
                     ...
                     if (permissions["delete"] && (this.options.showActions !== "none") )
                     {
      ...
      onNewList: function DataLists_onNewList(e, p_obj)
      {
         ...
         var fnPopulateItemTypes = function DataLists_onNewList_fnPopulateItemTypes(domId, formFieldId, p_form)
         {
            ...
            {
               list = this.options.listTypes[i];
               if( ((this.options.listTypesToShow === "") || (list.name.match(this.options.listTypesToShow)))
				&& (!list.name.match("^alvexdt:object$")) )
               {
         ...
         {
            itemKind: "type",
            itemId: this.options.listContainerType,
            ...
            Alfresco.util.Ajax.request(
            {
               ...
               successCallback:
               {
                  fn: function DataLists_onDeleteList_confirm_success(response, p_obj)
                  {
                     ...
                     {
                        window.location = this.options.dlUrlTemplate;
                        return;
                     }
