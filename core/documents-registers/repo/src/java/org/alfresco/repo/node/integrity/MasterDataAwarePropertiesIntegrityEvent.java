 ...
 * @author Derek Hulley
 */
public class MasterDataAwarePropertiesIntegrityEvent extends PropertiesIntegrityEvent
{
    private static Log logger = LogFactory.getLog(MasterDataAwarePropertiesIntegrityEvent.class);
    protected AlvexMasterDataService alvexMasterDataService;
    
    protected MasterDataAwarePropertiesIntegrityEvent(
            AlvexMasterDataService alvexMasterDataService,
            NodeService nodeService,
            DictionaryService dictionaryService,
            NodeRef nodeRef)
    {
        super(nodeService, dictionaryService, nodeRef);
            ...
            Collection<IntegrityRecord> eventResults)
    {
        ...
        for (PropertyDefinition propertyDef : propertyDefs)
        {
            ...
            {
                constraints.add(constraintDef.getConstraint());
            }
            // 'not normal' constraint from master data aspect
            if(parentRef != null)
			{
                Constraint c = alvexMasterDataService.getConstraint(parentRef, propertyQName.toPrefixString());
                if(c != null)
                    constraints.add(c);
			}
