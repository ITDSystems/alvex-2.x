...
(function()
{
    ...
    * @constructor
    */
   Alvex.DocRegObjectFinder = function Alfresco_ObjectFinder(htmlId, currentValueHtmlId)
   {
      ...
      this.options.objectRenderer = new Alvex.ObjectRenderer(this);
   ...
   YAHOO.extend(Alvex.DocRegObjectFinder, Alfresco.component.Base,
   {
       ...
       * @return {Alvex.DocRegObjectFinder} returns 'this' for method chaining
       */
      setOptions: function ObjectFinder_setOptions(obj)
      {
         Alvex.DocRegObjectFinder.superclass.setOptions.call(this, obj);
      ...
      {
         Alvex.DocRegObjectFinder.superclass.setMessages.call(this, obj);
      ...
      {
         Alfresco.util.getFileIcon.types["dl:dataList"] = "folder";
      ...
      destroy: function ObjectFinder_destroy()
      {
         ...
         Alvex.DocRegObjectFinder.superclass.destroy.call(this);
      },
            ...
            Alfresco.util.Ajax.jsonRequest({
               ...
               successCallback:
               {
                  fn:function(resp)
                  {
                     ...
                     var chooseItem = {"name": this.msg("alvex.uploader.chooseDocRegSite"), "nodeRef": "", "type": "st:site"};
                     navButton.set("label", this.options.objectRenderer.renderItem(chooseItem, 16, '<div><span class="item-icon">{icon}</span><span class="item-name">{name}</span></div>'));

                     var menuItem;
                     for (var i = 0; i < availableSites.length; i++)
                     {
                        var item = { "name": availableSites[i].title, 
                                     "nodeRef": availableSites[i].node.replace(/\/alfresco\/s\/api\/node\/workspace\//,'workspace:\/\/'), 
                                     "type": "st:site"};
                        menuItem = new YAHOO.widget.MenuItem(this.options.objectRenderer.renderItem(item, 16, '<span class="item-icon">{icon}</span><span class="item-name">{name}</span>'),
                        {
                           value: item.nodeRef
                        });
                        menuItem.cfg.addProperty("label",
...
(function()
{
    ...
    * @return {Alvex.ObjectRenderer} The new ObjectRenderer instance
    * @constructor
    */
   Alvex.ObjectRenderer = function(objectFinder)
   {
      this.objectFinder = objectFinder;
      
      Alvex.ObjectRenderer.superclass.constructor.call(this, "Alvex.ObjectRenderer", objectFinder.pickerId, ["button", "menu", "container", "datasource", "datatable"]);
      /**
       * Decoupled event listeners
       */
   ...
   YAHOO.extend(Alvex.ObjectRenderer, Alfresco.component.Base,
   {
      /**
       * Object container for initialization options
       *
      ...
      destroy: function ObjectRenderer_destroy()
      {
         ...
         Alvex.ObjectRenderer.superclass.destroy.call(this);
      },
      ...
      {   
         ...
         if ($hasEventInterest(this, args))
         {
            ...
            if (obj && obj.nodeRef)
            {
               this._updateItems(obj.nodeRef, obj.searchTerm);
            }
         }
      },
      ...
      _updateItems: function ObjectRenderer__updateItems(nodeRef, searchTerm)
      {
         ...
         var url = this._generatePickerChildrenUrlPath(nodeRef, searchTerm);//"api/alvex/documents-registers/site-registries?siteRef=" + nodeRef;
       ...
       */
      _generatePickerChildrenUrlPath: function ObjectRenderer__generatePickerChildrenUrlPath(nodeRef, type)
      {
         if( type === "regs" )
            return "api/alvex/documents-registers/site-registries?siteRef=" + nodeRef;
         else
            return $combine("api/forms/picker/", this.options.itemFamily, "/", nodeRef.replace("://", "/"), "children");
      },
