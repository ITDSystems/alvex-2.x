 ...
 * 
 * @namespace Alfresco
 * @class Alvex.OrgchartPeopleFinder
 */
(function()
{
   /**
    ...
    * @return {Alvex.OrgchartPeopleFinder} The new PeopleFinder instance
    * @constructor
    */
   Alvex.OrgchartPeopleFinder = function(htmlId)
   {
      Alvex.OrgchartPeopleFinder.superclass.constructor.call(this, "Alvex.OrgchartPeopleFinder", htmlId, ["button", "container", "datasource", "datatable", "json"]);
   ...
   YAHOO.lang.augmentObject(Alvex.OrgchartPeopleFinder,
   {
      VIEW_MODE_DEFAULT: "",
   ...
   YAHOO.lang.extend(Alvex.OrgchartPeopleFinder, Alfresco.component.Base,
   {
      /**
      ...
      options:
      {
          ...
          * @property viewMode
          * @type string
          * @default Alvex.OrgchartPeopleFinder.VIEW_MODE_DEFAULT
          */
         viewMode: Alvex.OrgchartPeopleFinder.VIEW_MODE_DEFAULT,
      ...
      {  
         ...
         if (this.options.viewMode == Alvex.OrgchartPeopleFinder.VIEW_MODE_COMPACT)
         {
            Dom.addClass(this.id + "-body", "compact");
            Dom.removeClass(this.id + "-results", "hidden");
         }
         else if (this.options.viewMode == Alvex.OrgchartPeopleFinder.VIEW_MODE_FULLPAGE)
         {
      ...
      _setupDataTable: function PeopleFinder__setupDataTable()
      {
          ...
          * These MUST be inline in order to have access to the Alvex.OrgchartPeopleFinder class (via the "me" variable).
          */
         var me = this;
         ...
         var renderCellDescription = function PeopleFinder_renderCellDescription(elCell, oRecord, oColumn, oData)
         {
            ...
            if (title.length !== 0)
            {
               if (me.options.viewMode == Alvex.OrgchartPeopleFinder.VIEW_MODE_COMPACT)
               {
            ...
            if (userStatus !== null && me.options.viewMode !== Alvex.OrgchartPeopleFinder.VIEW_MODE_COMPACT)
            {
         ...
         var renderCellActions = function PeopleFinder_renderCellActions(elCell, oRecord, oColumn, oData)
         {
            ...
            if (me.options.viewMode !== Alvex.OrgchartPeopleFinder.VIEW_MODE_FULLPAGE)
            {
         ...
         var renderCellRoles = function(elCell, oRecord, oColumn, oData)
         {
            Dom.setStyle(elCell.parentNode, "width", oColumn.width + "px");
         ...
         [
            { key: "avatar", label: "Avatar", sortable: false, formatter: renderCellAvatar, width: this.options.viewMode == Alvex.OrgchartPeopleFinder.VIEW_MODE_COMPACT ? 36 : 70 },
            { key: "person", label: "Description", sortable: false, formatter: renderCellDescription },
			{ key: "role", label: "Roles", sortable: false, formatter: renderCellRoles, width: 160 },
            { key: "actions", label: "Actions", sortable: false, formatter: renderCellActions, width: 80 }
         ];

         this.widgets.roleSelectors = [];
      ...
      getRoleLabel: function InvitationList_getRoleLabel(record)
      {
         ...
         {
            return record.getData("role").displayName;
         }
         return this.msg("alvex.orgchart.select_role");
      },
      ...
      onRoleSelect: function InvitationList_onRoleSelect(sType, aArgs, p_obj)
      {
         ...
         // update the button
         this.widgets.roleSelectors[record.getData("userName")].button.set("label", this.getRoleLabel(record));
      ...
      _setDefaultDataTableErrors: function PeopleFinder__setDefaultDataTableErrors(dataTable)
      {
         ...
         dataTable.set("MSG_EMPTY", msg("message.empty", "Alvex.OrgchartPeopleFinder"));
         dataTable.set("MSG_ERROR", msg("message.error", "Alvex.OrgchartPeopleFinder"));
      },
